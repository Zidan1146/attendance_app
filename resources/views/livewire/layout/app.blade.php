<div class="relative flex">
    <aside class="fixed h-screen p-4 text-white transition-all duration-150 bg-primary"
        ...
        <div class="absolute bottom-4 left-4 right-4">
            <button class="flex items-center w-full p-2 text-white rounded hover:bg-error">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="m-1 size-6">
                    <path fill-rule="evenodd"
    ...
    <!-- Main Content -->
    <div class="container p-4 transition-all duration-300" style="margin-left: {{ $isCollapsed ? '5rem' : '16rem' }}">
        @yield('content')
    </div>
</div>
<!-- <div class="relative flex">
    <input id="sidebar-drawer" type="checkbox" class="drawer-toggle" {{ $isCollapsed ? '' : 'checked' }} />
    <div class="drawer drawer-open">
        <aside class="drawer-side">
            <label for="sidebar-drawer" class="drawer-overlay"></label>
            <ul class="w-64 min-h-full p-4 menu bg-primary text-primary-content">
                <div class="flex items-center justify-between mb-4">
                    <button wire:click="toggleCollapse" class="btn btn-primary">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
                            <path fill-rule="evenodd"
                                d="M3 6.75A.75.75 0 0 1 3.75 6h16.5a.75.75 0 0 1 0 1.5H3.75A.75.75 0 0 1 3 6.75ZM3 12a.75.75 0 0 1 .75-.75h16.5a.75.75 0 0 1 0 1.5H3.75A.75.75 0 0 1 3 12Zm0 5.25a.75.75 0 0 1 .75-.75h16.5a.75.75 0 0 1 0 1.5H3.75a.75.75 0 0 1-.75-.75Z"
                                clip-rule="evenodd" />
                    ...
                    <h1 class="text-lg font-bold {{ $isCollapsed ? 'hidden' : 'block' }}">ABSENSI</h1>
                </div>

                <li class="{{ ($routeName ?: Route::currentRouteName()) === 'dashboard' ? 'bg-accent' : '' }}">
                    <a href="{{ route('dashboard') }}" wire:navigate class="flex items-center gap-2">
                        <svg class="size-6" viewBox="0 0 24 24" fill="currentColor">
                            <path fill-rule="evenodd"
                                d="M3 6a3 3 0 0 1 3-3h2.25a3 3 0 0 1 3 3v2.25a3 3 0 0 1-3 3H6a3 3 0 0 1-3-3V6Zm9.75 0a3 3 0 0 1 3-3H18a3 3 0 0 1 3 3v2.25a3 3 0 0 1-3 3h-2.25a3 3 0 0 1-3-3V6Z"
                                clip-rule="evenodd" />
                        </svg>
                        <span class="{{ $isCollapsed ? 'hidden' : 'inline' }}">Dashboard</span>
                    </a>
                </li>
                <li class="{{ ($routeName ?: Route::currentRouteName()) === 'absensi' ? 'bg-accent' : '' }}">
                    <a href="{{ route('absensi') }}" wire:navigate class="flex items-center gap-2">
                        <svg class="size-6" viewBox="0 0 24 24" fill="currentColor">
                            <path fill-rule="evenodd"
                                d="M19.916 4.626a.75.75 0 0 1 .208 1.04l-9 13.5a.75.75 0 0 1-1.154.114l-6-6a.75.75 0 0 1 1.06-1.06l5.353 5.353 8.493-12.74a.75.75 0 0 1 1.04-.207Z"
                                clip-rule="evenodd" />
                ...
                <li class="{{ ($routeName ?: Route::currentRouteName()) === 'worker' ? 'bg-accent' : '' }}">
                    <a href="{{ route('worker') }}" wire:navigate class="flex items-center gap-2">
                        <svg class="size-6" viewBox="0 0 24 24" fill="currentColor">
                            <path fill-rule="evenodd"
                                d="M7.5 6a4.5 4.5 0 1 1 9 0 4.5 4.5 0 0 1-9 0ZM3.751 20.105a8.25 8.25 0 0 1 16.498 0 .75.75 0 0 1-.437.695A18.683 18.683 0 0 1 12 22.5c-2.786 0-5.433-.608-7.812-1.7a.75.75 0 0 1-.437-.695Z"
                                clip-rule="evenodd" />
                ...
                <li class="{{ ($routeName ?: Route::currentRouteName()) === 'report' ? 'bg-accent' : '' }}">
                    <a href="{{ route('report') }}" wire:navigate class="flex items-center gap-2">
                        <svg class="size-6" viewBox="0 0 24 24" fill="currentColor">
                            <path
                                d="M5.625 1.5c-1.036 0-1.875.84-1.875 1.875v17.25c0 1.035.84 1.875 1.875 1.875h12.75c1.035 0 1.875-.84 1.875-1.875V12.75A3.75 3.75 0 0 0 16.5 9h-1.875a1.875 1.875 0 0 1-1.875-1.875V5.25A3.75 3.75 0 0 0 9 1.5H5.625Z" />
                        </svg>
                ...
                <li>
                    <button class="flex items-center w-full gap-2 btn btn-error">
                        <svg class="size-6" viewBox="0 0 24 24" fill="currentColor">
                            <path fill-rule="evenodd"
                                d="M16.5 3.75a1.5 1.5 0 0 1 1.5 1.5v13.5a1.5 1.5 0 0 1-1.5 1.5h-6a1.5 1.5 0 0 1-1.5-1.5V15a.75.75 0 0 0-1.5 0v3.75a3 3 0 0 0 3 3h6a3 3 0 0 0 3-3V5.25a3 3 0 0 0-3-3h-6a3 3 0 0 0-3 3V9A.75.75 0 1 0 9 9V5.25a1.5 1.5 0 0 1 1.5-1.5h6Z"
                                clip-rule="evenodd" />
                        </svg>
                        <span class="{{ $isCollapsed ? 'hidden' : 'inline' }}">Logout</span>
                    </button>
    ...
    <div class="container p-4">@yield('content')</div>
</div>
-->
